...
namespace AdventOfCode
{
    public static class Day3Task1
    {
        ...
        private static int mapBorder = mapSize / 2;
        
        public static int Solve()
        {
            var wire1Commands = inputWire1.Split(",");
            var wire2Commands = inputWire2.Split(",");
            
            var map1 = new Map(mapSize);
            var map2 = new Map(mapSize);
            
            PutWire(wire1Commands, map1, 1);
            PutWire(wire2Commands, map2, 2);

            return FindNearestCrossing(map1, map2);
        }

        private static int FindNearestCrossing(Map map1, Map map2)
        {
            var min = mapSize * 3;
            var minX = 0;
            var minY = 0;
            
            for (int i = -mapBorder; i < mapBorder; i++)
            {
                for (int j = -mapBorder; j < mapBorder; j++)
                {
                    if (map1[i, j] == CellState.Wire && map2[i, j] == CellState.Wire)
                    {
        ...
        private static void PutWire(string[] wireCommands, Map map, int wireNumber)
        {
            var currentX = 0;
            var currentY = 0;
            foreach (var command in wireCommands)
            {
                ...
                switch (direction)
                {
                    case 'R':
                        ...
                        {
                            currentX += 1;
                            map[currentX, currentY] = CellState.Wire;
                        }
    ...
    public class Map
    {
        ...
        public Map(int mapSize)
        {
            ...
            this.mapCenter = mapSize / 2;
            map = new CellState[mapSize, mapSize];
            mapOfLen = new int[mapSize, mapSize];
        }

        public CellState this[int x, int y]
        {
            get { return map[x + mapCenter, y + mapCenter]; }
            set { map[x + mapCenter, y + mapCenter] = value; }
        }

        public int GetWireLen(int x, int y)
        {
            return mapOfLen[x + mapCenter, y + mapCenter];
        }

        public void SetWireLen(int x, int y, int value)
        {
            mapOfLen[x + mapCenter, y + mapCenter] = value;
        }

        private CellState[,] map;
        private int[,] mapOfLen;
    }
